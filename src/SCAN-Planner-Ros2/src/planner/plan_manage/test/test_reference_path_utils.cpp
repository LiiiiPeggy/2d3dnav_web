...
TEST(ReferencePathUtils, RejectsEmptyAndSinglePointPaths)
{
  ...
  EXPECT_FALSE(scan_planner::prepareReferenceWaypoints(path, 0.4, 0.5, false, waypoints, &error));
...
TEST(ReferencePathUtils, DownsamplesInThreeDimensionsAndPreservesFinalPoint)
{
  ...
  std::vector<Eigen::Vector3d> waypoints;
  ASSERT_TRUE(scan_planner::prepareReferenceWaypoints(path, 0.4, 0.5, false, waypoints));
...
TEST(ReferencePathUtils, RejectsNonFiniteCoordinates)
{
  ...
  std::vector<Eigen::Vector3d> waypoints;
  EXPECT_FALSE(scan_planner::prepareReferenceWaypoints(path, 0.4, 0.5, false, waypoints));
  EXPECT_TRUE(waypoints.empty());
}

TEST(ReferencePathUtils, KeepsBodyHeightPathUnchanged)
{
  nav_msgs::msg::Path path;
  path.poses = {pose(0.0, 0.0, 0.4), pose(1.0, 0.0, 0.7)};
  std::vector<Eigen::Vector3d> waypoints;
  ASSERT_TRUE(scan_planner::prepareReferenceWaypoints(
      path, 0.4, 0.5, true, waypoints));
  EXPECT_NEAR(waypoints.front().z(), 0.4, 1e-9);
  EXPECT_NEAR(waypoints.back().z(), 0.7, 1e-9);
}
