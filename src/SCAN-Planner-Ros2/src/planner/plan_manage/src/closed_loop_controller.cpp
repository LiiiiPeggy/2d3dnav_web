...
public:
  ClosedLoopController() : Node("closed_loop_controller")
  {
    ...
    max_vx_ = std::min(
        declare_parameter<double>("max_vx", 0.75), kMaxLinearSpeedLimit);
    max_vy_ = std::min(
        declare_parameter<double>("max_vy", 0.35), kMaxLinearSpeedLimit);
    max_vyaw_ = std::min(declare_parameter<double>("max_vyaw", 0.75), kMaxVYawLimit);
    finish_dist_ = declare_parameter<double>("finish_dist", 0.15);
    odom_timeout_ = declare_parameter<double>("odom_timeout", 0.3);
    require_inputs_ready_ = declare_parameter<bool>("require_inputs_ready", false);
    if (odom_timeout_ <= 0.0)
      throw std::invalid_argument("odom_timeout must be positive");
    ...
    if (require_inputs_ready_)
      inputs_ready_sub_ = create_subscription<std_msgs::msg::Bool>(
          "inputs_ready", rclcpp::QoS(1).reliable().transient_local(),
          [this](const std_msgs::msg::Bool::ConstSharedPtr message) {
            inputs_ready_ = message->data;
          });
...
private:
  // These are hard real-robot limits.  A YAML or launch override may lower
  // them, but cannot command the dog faster than 0.75 m/s or 0.75 rad/s.
  static constexpr double kMaxLinearSpeedLimit = 0.75;
  static constexpr double kMaxVYawLimit = 0.75;
  ...
  void cmdCallback()
  {
    ...
    {
      publishExecutionFrozen(receive_traj_);
      publishStop();
      return;
    }
    const auto current_time = now();
    if (require_inputs_ready_ && !inputs_ready_)
    {
      RCLCPP_ERROR_THROTTLE(
          get_logger(), *get_clock(), 1000,
          "FAST-LIO odometry/cloud health check is not ready: forcing zero cmd_vel");
    ...
    if ((current_time - last_odom_time_).seconds() > odom_timeout_)
    {
      RCLCPP_ERROR_THROTTLE(
          get_logger(), *get_clock(), 1000,
          "Odometry timeout (>%0.2fs): forcing zero cmd_vel", odom_timeout_);
      publishExecutionFrozen(true);
      publishStop();
      last_update_time_ = current_time;
      return;
  ...
  rclcpp::Subscription<std_msgs::msg::Bool>::SharedPtr inputs_ready_sub_;
  rclcpp::TimerBase::SharedPtr cmd_timer_;
  bool receive_traj_{false};
  bool have_odom_{false};
  bool require_inputs_ready_{false};
  ...
  double max_vx_, max_vy_, max_vyaw_, finish_dist_, odom_timeout_;
};
}  // namespace scan_planner
