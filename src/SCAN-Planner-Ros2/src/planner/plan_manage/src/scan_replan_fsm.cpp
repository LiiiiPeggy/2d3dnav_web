...
namespace scan_planner
{
  ...
  void SCANReplanFSM::init(rclcpp::Node *node)
  {
    ...
    reference_path_z_is_body_ =
        load_parameter<bool>(node_, "fsm.reference_path_z_is_body", false);
  ...
  void SCANReplanFSM::rvizGoalCallback(const geometry_msgs::msg::PoseStamped::ConstSharedPtr &msg)
  {
    ...
    if (!msg->header.frame_id.empty() && msg->header.frame_id != self_inflation_frame_id_)
    {
      RCLCPP_WARN(node_->get_logger(),
                  "Ignore mode-1 goal in frame '%s'; expected '%s'",
  ...
  void SCANReplanFSM::waypointCallback(const nav_msgs::msg::Path::ConstSharedPtr &msg)
  {
    ...
    const auto &position = msg->poses[0].pose.position;
    if (!std::isfinite(position.x) || !std::isfinite(position.y))
    {
      RCLCPP_WARN(node_->get_logger(), "Ignore mode-1 goal with non-finite X/Y");
      return;
    }
    ...
    bool success = false;
    // Mode 1 is a flat local-planning mode.  RViz and the phone may attach a
    // ground or body Z to the clicked point, so deliberately ignore that Z and
    // snapshot the live body height.  This keeps the requested trajectory
    // horizontal even if FAST-LIO accumulated a small height offset after
    // startup, and it also allows maps whose valid ground Z is negative.
    end_pt_ << position.x, position.y, odom_pos_(2);
    RCLCPP_INFO(node_->get_logger(),
                "Mode-1 goal: [%.2f, %.2f], using current body z %.3f",
  ...
  void SCANReplanFSM::pathCallback(const nav_msgs::msg::Path::ConstSharedPtr &msg)
  {
    ...
    {
      RCLCPP_ERROR(node_->get_logger(),
                   "Ignoring initial_path in frame '%s'; expected '%s'",
                   msg->header.frame_id.c_str(), self_inflation_frame_id_.c_str());
      return;
    }
    ...
    if (!prepareReferenceWaypoints(*msg, body_height_, 0.5,
                                   reference_path_z_is_body_, waypoints, &path_error))
    {
