...
public:
  ...
  // Cartographer requires odometry timestamps to be strictly increasing.
  // Keep the last timestamp here so an invalid or repeated RF2O result can
  // never be forwarded to Cartographer.
  bool                                            odom_has_been_published{false};
  std::int64_t                                    last_published_odom_time_ns{0};
