...
void ImuProcess::IMU_init(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, int &N)
{
  ...
  {
    std::cerr << "Invalid mean acceleration during IMU initialization" << std::endl;
    return;
  }

  const V3D gravity_in_imu = -mean_acc / mean_acc_norm;
  if (gravity_align_world_)
  {
    // FAST-LIO normally keeps the initial IMU axes as its world frame and
    // stores gravity as a tilted S2 vector.  For a tilted physical sensor that
    // makes level floors tilted in /cloud_registered and in saved PCDs.
    const V3D gravity_in_world(0.0, 0.0, -1.0);
    Eigen::Quaterniond world_from_imu =
        Eigen::Quaterniond::FromTwoVectors(gravity_in_imu, gravity_in_world);
    world_from_imu.normalize();
    init_state.rot = SO3(world_from_imu);
    init_state.grav = S2(gravity_in_world * G_m_s2);
  }
  else
  {
    init_state.grav = S2(gravity_in_imu * G_m_s2);
  }
...
void ImuProcess::Process(const MeasureGroup &meas,  esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, PointCloudXYZI::Ptr cur_pcl_un_)
{
  ...
  if (imu_need_init_)
  {
    ...
    if (init_iter_num > MAX_INI_COUNT)
    {
      ...
      std::cout << "IMU Initial Done (gravity-aligned world: "
                << (gravity_align_world_ ? "true" : "false") << ")" << std::endl;
