...
class Nav2WebBridge(Node):
    ...
    def __init__(self):
        ...
        self.write_state_service = self.get_parameter(
            'write_state_service').value
        ...
        self._save_map_generation = 0
        self._save_map_pending = None
        self._nav_state = {
            'state': 'idle',
            'message': '等待导航目标',
        ...
        self.get_logger().info(
            ...
            f'TF {self.map_frame} -> {self.odom_frame} -> {self.base_frame}'
        )
    ...
    def _save_map(self, message: dict):
        ...
        if not self._write_state_client.service_is_ready():
            self._set_save_map_state(
                'unavailable',
                'Cartographer /write_state 服务未就绪',
            )
            return
        with self._state_lock:
            if self._save_map_pending is not None:
                raise ValueError('地图正在保存，请等待当前操作完成')
        ...
        grid_request = SaveMap.Request()
        grid_request.map_topic = self.map_topic
        grid_request.map_url = map_path
        grid_request.image_format = 'pgm'
        grid_request.map_mode = 'trinary'
        grid_request.free_thresh = 0.25
        grid_request.occupied_thresh = 0.65
        graph_request = WriteState.Request()
        graph_request.filename = pbstream_path
        # Mapping is still active when the Web save button is pressed. Include
        # unfinished submaps so the snapshot can later be loaded as a frozen
        # Cartographer graph without first terminating the trajectory.
        graph_request.include_unfinished_submaps = True

        with self._state_lock:
            self._save_map_generation += 1
            generation = self._save_map_generation
            self._save_map_pending = {
        ...
        self._set_save_map_state(
            'saving',
            '正在保存 YAML/PGM 和 Cartographer PBSTREAM',
            map_path,
        )

        try:
            grid_future = self._save_map_client.call_async(grid_request)
            graph_future = self._write_state_client.call_async(graph_request)
        except Exception as error:  # service transport errors are implementation-specific
            with self._state_lock:
                if (
                    self._save_map_pending is not None
                    and self._save_map_pending['generation'] == generation
                ):
                    self._save_map_pending = None
            self._set_save_map_state(
                'error', f'地图保存请求失败: {error}', map_path)
            return

        grid_future.add_done_callback(
            lambda future: self._save_grid_result(generation, future))
        graph_future.add_done_callback(
            lambda future: self._save_graph_result(generation, future))

    def _save_grid_result(self, generation: int, future):
        try:
            response = future.result()
            succeeded = bool(response.result)
            message = '' if succeeded else 'map_saver 返回失败'
        except Exception as error:  # service transport errors are implementation-specific
            succeeded = False
            message = str(error)
        self._complete_map_save_component(
            generation, 'grid', succeeded, message)

    def _save_graph_result(self, generation: int, future):
        try:
            response = future.result()
            status = response.status
            succeeded = int(status.code) == 0
            message = str(status.message).strip()
            if not succeeded and not message:
                message = f'/write_state 状态码 {status.code}'
        except Exception as error:  # service transport errors are implementation-specific
            succeeded = False
            message = str(error)
        self._complete_map_save_component(
            generation, 'graph', succeeded, message)

    def _complete_map_save_component(
        self,
        generation: int,
        component: str,
        succeeded: bool,
        ...
        with self._state_lock:
            pending = self._save_map_pending
            ...
            path = pending['path']
            pbstream_path = pending['pbstream_path']
            grid = pending['grid']
            graph = pending['graph']
            if grid is None or graph is None:
                waiting_for = (
                    'YAML/PGM' if grid is None else 'Cartographer PBSTREAM')
            else:
                waiting_for = None
                self._save_map_pending = None
        ...
        if waiting_for is not None:
            self._set_save_map_state(
                'saving', f'正在等待 {waiting_for} 保存完成', path)
            return

        if grid['succeeded'] and graph['succeeded']:
            self._set_save_map_state(
                'succeeded',
                f'地图已完整保存: {path}.yaml + {pbstream_path}',
                path,
            )
        else:
            failures = []
            if not grid['succeeded']:
                failures.append(f'YAML/PGM: {grid["message"] or "失败"}')
            if not graph['succeeded']:
                failures.append(
                    f'PBSTREAM: {graph["message"] or "失败"}')
            self._set_save_map_state(
                'error', '地图保存不完整；' + '；'.join(failures), path)

        # Refresh the navigation map selector immediately. It only exposes a
        # Cartographer map after the same-name .yaml and .pbstream both exist.
        if self._launch_manager is not None:
            self._servers.websocket.send_json(self._launch_manager.snapshot())
        ...
        with self._state_lock:
            ...
            return {
                ...
                'save_map_ready': (
                    ...
                    and self._write_state_client.service_is_ready()
                ),
                'map_saver_ready': self._save_map_client.service_is_ready(),
                'cartographer_write_state_ready': (
                    self._write_state_client.service_is_ready()
                ),
