...
const state = {
  ...
  launchScope: 'mapping',
  launchSelections: { mapping: null, navigation: null },
  ready: {
    map: false, tf: false, nav2: false, scan: false,
    saveMap: false, mapSaver: false, writeState: false,
    localizationReset: false,
  },
...
const LAYOUT_STORAGE_KEY = 'dog-nav2-layout-v1';
const DEFAULT_LAYOUT_PREFERENCES = Object.freeze({
  portraitMapRatio: 0.45,
  landscapeMapRatio: 0.60,
  controlScale: 1,
  showLandscapeSummary: true,
  summaryHeightRatio: 0.28,
  poseWidthRatio: 0.50,
});
let layoutPreferences = { ...DEFAULT_LAYOUT_PREFERENCES };
let savedLayoutPreferences = { ...DEFAULT_LAYOUT_PREFERENCES };
...
function setAppTab(tab) {
  const currentProfile = $('launchProfile')?.value;
  if (currentProfile && ['mapping', 'navigation'].includes(state.launchScope)) {
    state.launchSelections[state.launchScope] = currentProfile;
  }
  state.activeTab = tab;
  if (tab === 'mapping' || tab === 'nav') {
    state.launchScope = tab === 'mapping' ? 'mapping' : 'navigation';
    mountLaunchCard(state.launchScope);
    updateLaunchCard();
  }
...
function scrollLayersCardIntoView() {
  setAppTab('settings');
  const card = document.querySelector('.layers-card');
  if (card) card.scrollIntoView({ behavior: 'smooth', block: 'start' });
}

function profileLaunchScope(profile) {
  return profile?.requires_map ? 'navigation' : 'mapping';
}

function mountLaunchCard(scope) {
  const slot = $(scope === 'navigation'
    ? 'navigationLaunchSlot' : 'mappingLaunchSlot');
  const card = $('launchCard');
  if (slot && card && card.parentElement !== slot) slot.append(card);
}

function organizeWorkflowPanels() {
  const mappingPanel = document.querySelector('[data-tab-panel="mapping"]');
  const navigationPanel = document.querySelector('[data-tab-panel="nav"]');
  const statusPanel = document.querySelector('[data-tab-panel="status"]');
  const settingsPanel = document.querySelector('[data-tab-panel="settings"]');

  mappingPanel?.append(document.querySelector('.mapping-card'));
  navigationPanel?.append(document.querySelector('.workflow-card'));
  statusPanel?.append(
    document.querySelector('.match-card'),
    document.querySelector('.velocity-card'),
  );
  settingsPanel?.append(
    document.querySelector('.layers-card'),
    document.querySelector('.inflation-card'),
  );
  mountLaunchCard('mapping');
}
...
function sanitizedLayoutPreferences(value) {
  const candidate = value && typeof value === 'object' ? value : {};
  return {
    portraitMapRatio: clampLayoutValue(
      candidate.portraitMapRatio ?? DEFAULT_LAYOUT_PREFERENCES.portraitMapRatio,
      0.28,
      0.72,
    ),
    landscapeMapRatio: clampLayoutValue(
      candidate.landscapeMapRatio ?? DEFAULT_LAYOUT_PREFERENCES.landscapeMapRatio,
      0.38,
      0.78,
    ),
    controlScale: clampLayoutValue(
      candidate.controlScale ?? DEFAULT_LAYOUT_PREFERENCES.controlScale,
      0.8,
      1.3,
    ),
    showLandscapeSummary: candidate.showLandscapeSummary === undefined
      ? DEFAULT_LAYOUT_PREFERENCES.showLandscapeSummary
      : Boolean(candidate.showLandscapeSummary),
    summaryHeightRatio: clampLayoutValue(
      candidate.summaryHeightRatio ?? DEFAULT_LAYOUT_PREFERENCES.summaryHeightRatio,
      0.18,
      0.48,
    ),
    poseWidthRatio: clampLayoutValue(
      candidate.poseWidthRatio ?? DEFAULT_LAYOUT_PREFERENCES.poseWidthRatio,
      0.30,
      0.70,
    ),
  };
...
function loadLayoutPreferences() {
  try {
    ...
    if (!stored) return { ...DEFAULT_LAYOUT_PREFERENCES };
    return sanitizedLayoutPreferences(JSON.parse(stored));
  } catch (_error) {
...
function isPortraitLayout() {
  return document.documentElement.classList.contains('portrait-layout');
}

function currentMapRatio() {
  return isPortraitLayout()
    ? layoutPreferences.portraitMapRatio
    : layoutPreferences.landscapeMapRatio;
}

function currentLayoutGeometry() {
  ...
  return {
    ...
    usableWidth: Math.max(1, workspace.clientWidth - paddingLeft - paddingRight - columnGap),
    usableHeight: Math.max(1, workspace.clientHeight - paddingTop - paddingBottom - rowGap),
    columnGap,
    rowGap,
  };
}
...
function applyControlScale() {
  ...
  const shortLandscape = !portrait && height <= 570;
  let baseControlSize;
  let baseNavHeight;
  if (portrait) {
    baseControlSize = clampLayoutValue(width * 0.105, 40, 44);
    baseNavHeight = clampLayoutValue(height * 0.08, 50, 66);
  } else if (shortLandscape) {
    baseControlSize = clampLayoutValue(height * 0.11, 38, 46);
    baseNavHeight = clampLayoutValue(height * 0.17, 46, 56);
  } else {
    baseControlSize = clampLayoutValue(Math.min(width, height) * 0.07, 42, 54);
    baseNavHeight = clampLayoutValue(height * 0.085, 52, 72);
  }

  const scale = layoutPreferences.controlScale;
  const mapPanelWidth = document.querySelector('.map-panel')?.clientWidth || width;
  const widthLimit = Math.max(34, (mapPanelWidth - 38) / 5);
  const controlSize = clampLayoutValue(
    Math.min(baseControlSize * scale, widthLimit),
    32,
    68,
  );
  const compactControls = portrait || shortLandscape;
  const labelWidth = compactControls
    ? controlSize : clampLayoutValue(controlSize * 1.34, controlSize, 86);

  root.classList.add('user-sized-controls');
  root.classList.toggle('narrow-map-pane', mapPanelWidth < 270);
  root.style.setProperty('--map-control-size', `${controlSize.toFixed(1)}px`);
  root.style.setProperty('--map-control-label-width', `${labelWidth.toFixed(1)}px`);
  root.style.setProperty('--user-map-icon-size', `${(controlSize * 0.6).toFixed(1)}px`);
  root.style.setProperty('--user-map-label-font-size', `${clampLayoutValue(11 * scale, 9, 15).toFixed(1)}px`);
  root.style.setProperty('--user-map-tool-icon-size', `${(controlSize * 0.43).toFixed(1)}px`);
  root.style.setProperty('--user-nav-height', `${clampLayoutValue(baseNavHeight * scale, 42, 86).toFixed(1)}px`);
  root.style.setProperty('--user-nav-icon-size', `${clampLayoutValue(19 * scale, 15, 28).toFixed(1)}px`);
}

function positionLayoutResizeHandle() {
  const handle = $('layoutResizeHandle');
  const workspace = document.querySelector('.workspace');
  const mapColumn = document.querySelector('.map-column');
  const sidePanel = document.querySelector('.side-panel');
  if (!handle || !workspace || !mapColumn || !sidePanel || state.mapExpanded) return;
  const workspaceRect = workspace.getBoundingClientRect();
  const mapRect = mapColumn.getBoundingClientRect();
  const panelRect = sidePanel.getBoundingClientRect();
  if (isPortraitLayout()) {
    handle.style.left = `${(mapRect.left + mapRect.right) / 2 - workspaceRect.left}px`;
    handle.style.top = `${(mapRect.bottom + panelRect.top) / 2 - workspaceRect.top}px`;
  } else {
    handle.style.left = `${(mapRect.right + panelRect.left) / 2 - workspaceRect.left}px`;
    handle.style.top = `${(mapRect.top + mapRect.bottom) / 2 - workspaceRect.top}px`;
  }

  if (!isPortraitLayout() && layoutPreferences.showLandscapeSummary) {
    const summary = document.querySelector('.summary-strip');
    const mapPanel = document.querySelector('.map-panel');
    const poseCard = document.querySelector('.pose-card');
    const navCard = document.querySelector('.nav-card');
    const heightHandle = $('summaryHeightHandle');
    const widthHandle = $('summaryWidthHandle');
    const columnRect = mapColumn.getBoundingClientRect();
    const summaryRect = summary?.getBoundingClientRect();
    const mapPanelRect = mapPanel?.getBoundingClientRect();
    const poseRect = poseCard?.getBoundingClientRect();
    const navRect = navCard?.getBoundingClientRect();
    if (summaryRect && mapPanelRect && heightHandle) {
      heightHandle.style.left = `${(mapPanelRect.left + mapPanelRect.right) / 2 - columnRect.left}px`;
      heightHandle.style.top = `${(mapPanelRect.bottom + summaryRect.top) / 2 - columnRect.top}px`;
    }
    if (summaryRect && poseRect && navRect && widthHandle) {
      widthHandle.style.left = `${(poseRect.right + navRect.left) / 2 - summaryRect.left}px`;
    }
  }
}
...
function updateLayoutControls() {
  ...
  const portrait = isPortraitLayout();
  const split = portrait
    ? layoutPreferences.portraitMapRatio
    : layoutPreferences.landscapeMapRatio;
  $('layoutOrientation').textContent = portrait ? '当前：竖屏' : '当前：横屏';
  $('layoutSplitLabel').textContent = portrait ? '地图窗口高度' : '地图窗口宽度';
  $('layoutSplit').min = portrait ? '28' : '38';
  $('layoutSplit').max = portrait ? '72' : '78';
  $('layoutSplit').value = String(Math.round(split * 100));
  $('layoutSplitValue').textContent = `${Math.round(split * 100)}%`;
  $('layoutControlScale').value = String(Math.round(layoutPreferences.controlScale * 100));
  $('layoutControlScaleValue').textContent = `${Math.round(layoutPreferences.controlScale * 100)}%`;
  $('layoutShowSummary').checked = layoutPreferences.showLandscapeSummary;
  $('layoutSummaryHeight').value = String(Math.round(layoutPreferences.summaryHeightRatio * 100));
  $('layoutSummaryHeightValue').textContent = `${Math.round(layoutPreferences.summaryHeightRatio * 100)}%`;
  $('layoutPoseWidth').value = String(Math.round(layoutPreferences.poseWidthRatio * 100));
  $('layoutPoseWidthValue').textContent = `${Math.round(layoutPreferences.poseWidthRatio * 100)}%`;
  $('layoutSummaryControls').classList.toggle('portrait-disabled', portrait);
  $('layoutSummaryControls').title = portrait
    ? '此区域仅在横屏显示，设置仍会保存' : '';

  const changed = JSON.stringify(layoutPreferences) !== JSON.stringify(savedLayoutPreferences);
  $('layoutSaveHint').classList.toggle('unsaved', changed);
  $('layoutSaveHint').textContent = changed
    ? '布局已调整但尚未保存'
    : '布局已保存；也可以直接拖动地图与控制面板之间的分隔条';
}

function applyLayoutPreferences() {
  if (!layoutInitialized) return;
  const geometry = currentLayoutGeometry();
  const portrait = isPortraitLayout();
  const summaryVisible = !portrait
    && layoutPreferences.showLandscapeSummary
    && !state.mapExpanded;
  document.documentElement.classList.toggle('summary-visible', summaryVisible);
  if (state.mapExpanded) {
    geometry.workspace.style.gridTemplateColumns = 'minmax(0, 1fr)';
    geometry.workspace.style.gridTemplateRows = 'minmax(0, 1fr)';
    return;
  }

  if (portrait) {
    const mapHeight = Math.round(geometry.usableHeight * layoutPreferences.portraitMapRatio);
    geometry.workspace.style.gridTemplateColumns = 'minmax(0, 1fr)';
    geometry.workspace.style.gridTemplateRows = `${mapHeight}px minmax(0, 1fr)`;
  } else {
    const mapWidth = Math.round(geometry.usableWidth * layoutPreferences.landscapeMapRatio);
    geometry.workspace.style.gridTemplateRows = 'minmax(0, 1fr)';
    geometry.workspace.style.gridTemplateColumns = `${mapWidth}px minmax(0, 1fr)`;
  }
  ...
  const summary = document.querySelector('.summary-strip');
  if (summaryVisible) {
    const columnStyle = getComputedStyle(mapColumn);
    const columnGap = parseFloat(columnStyle.rowGap) || 0;
    const usableHeight = Math.max(1, mapColumn.clientHeight - columnGap);
    const summaryHeight = Math.round(
      usableHeight * layoutPreferences.summaryHeightRatio,
    );
    mapColumn.style.gridTemplateRows = `minmax(0, 1fr) ${summaryHeight}px`;

    const summaryStyle = getComputedStyle(summary);
    const summaryGap = parseFloat(summaryStyle.columnGap) || 0;
    const usableWidth = Math.max(1, summary.clientWidth - summaryGap);
    const poseWidth = Math.round(usableWidth * layoutPreferences.poseWidthRatio);
    summary.style.gridTemplateColumns = `${poseWidth}px minmax(0, 1fr)`;
  } else {
    mapColumn.style.gridTemplateRows = 'minmax(0, 1fr)';
...
function initializeLayoutControls() {
  syncViewportHeight();
  savedLayoutPreferences = loadLayoutPreferences();
  layoutPreferences = { ...savedLayoutPreferences };
  layoutInitialized = true;

  $('layoutSplit').addEventListener('input', (event) => {
    const ratio = Number(event.target.value) / 100;
    if (isPortraitLayout()) layoutPreferences.portraitMapRatio = ratio;
    else layoutPreferences.landscapeMapRatio = ratio;
    applyLayoutPreferences();
    markLayoutUnsaved();
  });
  $('layoutControlScale').addEventListener('input', (event) => {
    layoutPreferences.controlScale = Number(event.target.value) / 100;
    applyLayoutPreferences();
    markLayoutUnsaved();
  });
  $('layoutShowSummary').addEventListener('change', (event) => {
    layoutPreferences.showLandscapeSummary = Boolean(event.target.checked);
    applyLayoutPreferences();
    markLayoutUnsaved();
  });
  $('layoutSummaryHeight').addEventListener('input', (event) => {
    layoutPreferences.summaryHeightRatio = Number(event.target.value) / 100;
    applyLayoutPreferences();
    markLayoutUnsaved();
  });
  $('layoutPoseWidth').addEventListener('input', (event) => {
    layoutPreferences.poseWidthRatio = Number(event.target.value) / 100;
    applyLayoutPreferences();
    markLayoutUnsaved();
  });
  $('layoutSave').addEventListener('click', () => {
    try {
      ...
      savedLayoutPreferences = { ...layoutPreferences };
      updateLayoutControls();
      showToast('布局已保存在当前设备');
    } catch (_error) {
      showToast('布局保存失败：浏览器存储不可用');
    }
  });
  $('layoutReset').addEventListener('click', () => {
    layoutPreferences = { ...DEFAULT_LAYOUT_PREFERENCES };
    savedLayoutPreferences = { ...DEFAULT_LAYOUT_PREFERENCES };
    try {
      window.localStorage.removeItem(LAYOUT_STORAGE_KEY);
    } catch (_error) {
      // The default layout can still be applied for the current session.
    }
    applyLayoutPreferences();
    showToast('已恢复默认布局');
  });
  ...
  handle.addEventListener('pointerdown', (event) => {
    ...
    layoutDrag = { pointerId: event.pointerId };
  ...
  handle.addEventListener('pointermove', (event) => {
    ...
    const geometry = currentLayoutGeometry();
    if (isPortraitLayout()) {
      const value = (event.clientY - geometry.contentTop - geometry.rowGap / 2)
        / geometry.usableHeight;
      layoutPreferences.portraitMapRatio = clampLayoutValue(value, 0.28, 0.72);
    } else {
      const value = (event.clientX - geometry.contentLeft - geometry.columnGap / 2)
        / geometry.usableWidth;
      layoutPreferences.landscapeMapRatio = clampLayoutValue(value, 0.38, 0.78);
    }
    applyLayoutPreferences();
    markLayoutUnsaved();
  });
  const finishLayoutDrag = (event) => {
    if (!layoutDrag || layoutDrag.pointerId !== event.pointerId) return;
    layoutDrag = null;
    handle.classList.remove('dragging');
    document.body.classList.remove('layout-resizing');
  };
  handle.addEventListener('pointerup', finishLayoutDrag);
  handle.addEventListener('pointercancel', finishLayoutDrag);
  handle.addEventListener('dblclick', () => {
    if (isPortraitLayout()) {
      layoutPreferences.portraitMapRatio = DEFAULT_LAYOUT_PREFERENCES.portraitMapRatio;
    } else {
      layoutPreferences.landscapeMapRatio = DEFAULT_LAYOUT_PREFERENCES.landscapeMapRatio;
    }
    applyLayoutPreferences();
    markLayoutUnsaved();
  });

  const bindSummaryHandle = (resizeHandle, kind) => {
    resizeHandle.addEventListener('pointerdown', (event) => {
      if (isPortraitLayout() || !layoutPreferences.showLandscapeSummary) return;
      event.preventDefault();
      resizeHandle.setPointerCapture(event.pointerId);
      layoutDrag = { pointerId: event.pointerId, kind };
    ...
    resizeHandle.addEventListener('pointermove', (event) => {
      if (!layoutDrag
          || layoutDrag.pointerId !== event.pointerId
          || layoutDrag.kind !== kind) return;
      if (kind === 'summary-height') {
        const mapColumn = document.querySelector('.map-column');
        const style = getComputedStyle(mapColumn);
        const gap = parseFloat(style.rowGap) || 0;
        const rect = mapColumn.getBoundingClientRect();
        const usableHeight = Math.max(1, mapColumn.clientHeight - gap);
        const summaryHeight = rect.bottom - event.clientY - gap / 2;
        layoutPreferences.summaryHeightRatio = clampLayoutValue(
          summaryHeight / usableHeight,
          0.18,
          0.48,
        );
      } else {
        ...
        const usableWidth = Math.max(1, summary.clientWidth - gap);
        const poseWidth = event.clientX - rect.left - gap / 2;
        layoutPreferences.poseWidthRatio = clampLayoutValue(
          poseWidth / usableWidth,
          0.30,
          0.70,
        );
      }
    ...
    resizeHandle.addEventListener('dblclick', () => {
      if (kind === 'summary-height') {
        layoutPreferences.summaryHeightRatio = DEFAULT_LAYOUT_PREFERENCES.summaryHeightRatio;
      } else {
        layoutPreferences.poseWidthRatio = DEFAULT_LAYOUT_PREFERENCES.poseWidthRatio;
      }
      applyLayoutPreferences();
      markLayoutUnsaved();
    });
  };
  bindSummaryHandle($('summaryHeightHandle'), 'summary-height');
  bindSummaryHandle($('summaryWidthHandle'), 'summary-width');
...
function notifyAndroidConnection(connected) {
  try {
    window.Nav2Android?.connectionChanged(Boolean(connected));
  } catch (_error) {
    // A normal browser has no Android bridge; the Web UI still works.
  }
}
...
function handleMessage(message) {
  switch (message.type) {
    ...
    case 'telemetry':
      ...
      state.ready.mapSaver = Boolean(message.map_saver_ready);
      state.ready.writeState = Boolean(message.cartographer_write_state_ready);
...
function syncLaunchOptions() {
  ...
  const profiles = (state.launchControl.profiles || []).filter(
    (profile) => profileLaunchScope(profile) === state.launchScope,
  );
  ...
  if (profiles.length === 0) {
    ...
    const savedProfile = state.launchSelections[state.launchScope];
    const activeProfile = state.launchControl.active?.profile_id;
    const preferred = [savedProfile, previousProfile, activeProfile]
      .find((profileId) => profiles.some((profile) => profile.id === profileId))
      || profiles[0].id;
    profileSelect.value = preferred;
    state.launchSelections[state.launchScope] = preferred;
  }
...
function updateLaunchCard() {
  ...
  const profile = selectedLaunchProfile();
  const maps = compatibleLaunchMaps(profile);
  const isNavigation = state.launchScope === 'navigation';
  const scopeLabel = isNavigation ? '导航' : '建图';
  $('launchCardTitle').textContent = `${scopeLabel} Launch 控制`;
  $('launchProfileLabelText').textContent = `选择${scopeLabel}入口`;
  $('launchStart').textContent = `启动${scopeLabel}`;
  const stateLabels = {
  ...
  const activeProfile = (control.profiles || []).find(
    (candidate) => candidate.id === active?.profile_id,
  );
  const activeOtherScope = activeProfile
    && profileLaunchScope(activeProfile) !== state.launchScope;
  $('launchSecurityHint').textContent = control.enabled
    ? activeOtherScope
      ? `当前正在运行${profileLaunchScope(activeProfile) === 'navigation' ? '导航' : '建图'}流程；请先停止，再启动${scopeLabel}`
...
function updateMappingCard() {
  ...
  if (!state.ready.mapSaver) saveHint = '等待 /map_saver/save_map';
  else if (!state.ready.writeState) saveHint = '等待 Cartographer /write_state';
...
function refreshResponsiveLayout() {
  ...
  responsiveLayoutFrame = requestAnimationFrame(() => {
    ...
    if (layoutInitialized) applyLayoutPreferences();
    resizeCanvas();
  });
}
...
document.querySelectorAll('[data-localizer]').forEach((button) => {
  button.addEventListener('click', () => {
    ...
    state.launchSelections.navigation = state.localizationChoice === 'cartographer'
      ? 'cartographer_mppi_navigation' : 'nav2_mppi_amcl';
    updateLocalizationSelector();
    updateActionAvailability();
    if (state.launchScope === 'navigation') updateLaunchCard();
    const active = activeLocalizationType();
...
$('openLayersPanel')?.addEventListener('click', scrollLayersCardIntoView);
...
$('launchProfile').addEventListener('change', () => {
  state.launchSelections[state.launchScope] = $('launchProfile').value;
  updateLaunchCard();
});
...
window.addEventListener('orientationchange', () => {
  setTimeout(refreshResponsiveLayout, 80);
  setTimeout(refreshResponsiveLayout, 360);
});
...
if ('ResizeObserver' in window) {
  const canvasResizeObserver = new ResizeObserver(refreshResponsiveLayout);
